...
def _fmt(secs: float) -> str:
    """Format seconds as a human-readable string, e.g. '1h 23m 04s' or '45.3s'."""
    secs = max(0.0, secs)
    if secs < 60:
        return f"{secs:.1f}s"
...
def convert_dataset(
    ...
    sep = "=" * (col + 22)
    print(f"\n{sep}")
    print(f"  {'STEP':<{col}}  TIME")
    print(sep)
    for step, secs in step_times:
        print(f"  {step:<{col}}  {_fmt(secs):>10}")
    print(sep)
    print(f"  {'TOTAL':<{col}}  {_fmt(total_elapsed):>10}")
    print(f"{sep}\n")
